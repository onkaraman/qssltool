...
namespace SSLLabsApiWrapper.Domain
{
	class ResponsePopulation
	{
		...
		{
			if (webResponse.Payload == null) { throw new Exception("webResponse payload is null. Unable to bind null."); }

			infoModel = JsonConvert.DeserializeObject<Info>(webResponse.Payload, JsonSerializerSettings);
		...
		{
			analyzeModel = JsonConvert.DeserializeObject<Analyze>(webResponse.Payload, JsonSerializerSettings);
		...
		{
			endpointModel = JsonConvert.DeserializeObject<Endpoint>(webResponse.Payload, JsonSerializerSettings);
		...
		{
			statusCodes = JsonConvert.DeserializeObject<StatusCodes>(webResponse.Payload, JsonSerializerSettings);
		...
		public Wrapper PopulateWrapper(Wrapper wrapper, WebResponseModel webResponse)
		{
			...
			wrapper.ApiRawResponse = webResponse.Payload;
